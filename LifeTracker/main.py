...
class DB:
    """
    SQLite storage for LifeTracker.
    """
    ...
    # ------------- TASKS API -------------
    def add_task(self, title, description, due_ts, priority="Low", category=None) -> int:
        self.cur.execute(
            "INSERT INTO tasks(title, description, due_ts, priority, status, category, created_at) VALUES (?,?,?,?,0,?, datetime('now'))",
    ...
    def update_task(self, task_id: int, **kwargs):
        fields = []
        values = []
        for k in ["title","description","due_ts","priority","category","status"]:
    ...
    def list_tasks(self, scope="all", category=None, date_from=None, date_to=None, search=None,
                   order_by="COALESCE(due_ts, created_at)", order_dir="ASC"):
        where = []
        params = []
        if scope == "today":
        ...
        if date_from and date_to:
            where.append("date(COALESCE(due_ts, created_at)) BETWEEN ? AND ?"); params += [date_from, date_to]
        if search:
            like = f"%{search.strip()}%"; where.append("(title LIKE ? OR description LIKE ?)"); params += [like, like]
    ...
    # ------------- EXPENSES API -------------
    def add_expense(self, dt: str, category, amount: float, note):
        self.cur.execute(
            "INSERT INTO expenses(dt, category, amount, note, created_at) VALUES (?,?,?,?, datetime('now'))",
            (dt, category, float(amount), note)
        )
        self.conn.commit()
        return int(self.cur.lastrowid)

    def list_expenses(self, *, date_from=None, date_to=None, category=None, search=None,
                      order_by="dt", order_dir="DESC"):
        where = []
        params = []
        if date_from and date_to:
            where.append("date(dt) BETWEEN ? AND ?"); params += [date_from, date_to]
        if category:
            where.append("category=?"); params.append(category)
        if search:
            like = f"%{search.strip()}%"; where.append("(category LIKE ? OR note LIKE ?)"); params += [like, like]
    ...
    # ------------- EVENTS API -------------
    def add_event(self, title, start_ts, end_ts=None, category=None, note=None) -> int:
        self.cur.execute(
            "INSERT INTO events(title, start_ts, end_ts, category, note, created_at) VALUES (?,?,?,?,?, datetime('now'))",
            (title, start_ts, end_ts, category, note)
        )
        self.conn.commit()
        return int(self.cur.lastrowid)

    def list_events(self, *, date_from=None, date_to=None, category=None, search=None,
                    order_by="start_ts", order_dir="DESC"):
        where = []; params = []
        if date_from and date_to:
            where.append("date(start_ts) BETWEEN ? AND ?"); params += [date_from, date_to]
        if category:
            where.append("category=?"); params.append(category)
        if search:
            like = f"%{search.strip()}%"; where.append("(title LIKE ? OR note LIKE ?)"); params += [like, like]
    ...
    def add_note(self, title: str, body: str) -> int:
        self.cur.execute("INSERT INTO notes(title, body, created_at) VALUES (?, ?, datetime('now'))", (title, body))
        self.conn.commit()
        return int(self.cur.lastrowid)
